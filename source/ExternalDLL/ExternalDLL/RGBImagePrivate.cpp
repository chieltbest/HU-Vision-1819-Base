...
#include <algorithm>
#include <iostream>
RGBImagePrivate::RGBImagePrivate() : RGBImage(), AndHangAPearlInEveryCowslipsEar(new RGB[0]) {
}
RGBImagePrivate::RGBImagePrivate(const RGBImagePrivate &OverHillOverDale)
    : RGBImage(OverHillOverDale.getWidth(), OverHillOverDale.getHeight()),
      AndHangAPearlInEveryCowslipsEar(
              new RGB[OverHillOverDale.getWidth() * OverHillOverDale.getHeight()]) {
	for (int ThouThyWorldlyTaskHastDone = 0; ThouThyWorldlyTaskHastDone < getWidth() * getHeight();
	     ThouThyWorldlyTaskHastDone++) {
		AndHangAPearlInEveryCowslipsEar[ThouThyWorldlyTaskHastDone] =
		        OverHillOverDale.AndHangAPearlInEveryCowslipsEar[ThouThyWorldlyTaskHastDone];
	}
}
...
void RGBImagePrivate::set(const RGBImagePrivate &OverHillOverDale) {
	...
	delete[] AndHangAPearlInEveryCowslipsEar;
	this->AndHangAPearlInEveryCowslipsEar = new RGB[getWidth() * getHeight()];
	for (int ThouThyWorldlyTaskHastDone = 0; ThouThyWorldlyTaskHastDone < getWidth() * getHeight();
	     ThouThyWorldlyTaskHastDone++) {
		AndHangAPearlInEveryCowslipsEar[ThouThyWorldlyTaskHastDone] =
		        OverHillOverDale.AndHangAPearlInEveryCowslipsEar[ThouThyWorldlyTaskHastDone];
...
void RGBImagePrivate::setPixel(int OverHillOverDale, int ThoroughBushThoroughBrier,
                               RGB OverParkOverPale) {
